...
if [ "$DEVICE_CHOICE" = "2" ]; then
    ...
    if grep -qi microsoft /proc/version 2>/dev/null; then
        # Running in WSL - get Windows host IP with improved filtering
        if command -v powershell.exe &> /dev/null; then
            # Get all Windows IPs with their interface info
            ALL_IPS=$(powershell.exe "Get-NetIPAddress -AddressFamily IPv4 | Select-Object -ExpandProperty IPAddress" 2>/dev/null | tr -d '\r')
            
            # Filter out unwanted IPs (loopback, link-local, WSL, common virtual adapters)
            FILTERED_IPS=$(echo "$ALL_IPS" | \
                           grep -v "^127\." | \
                           grep -v "^169\.254\." | \
                           grep -v "^172\.27\." | \
                           grep -v "^172\.16\." | \
            ...
            # Try to get IP from interface with default gateway
            DEFAULT_GATEWAY_IP=$(powershell.exe "Get-NetRoute -DestinationPrefix '0.0.0.0/0' | Get-NetIPAddress -AddressFamily IPv4 | Select-Object -ExpandProperty IPAddress" 2>/dev/null | tr -d '\r' | head -n 1)
            
            if [ -n "$DEFAULT_GATEWAY_IP" ] && [ "$DEFAULT_GATEWAY_IP" != "127.0.0.1" ]; then
                HOST_IP="$DEFAULT_GATEWAY_IP"
            else
                # Fallback: Prefer home router IPs (192.168.0.x or 192.168.1.x or 192.168.10.x)
                HOME_IP=$(echo "$FILTERED_IPS" | grep -E "^192\.168\.(0|1|10)\." | head -n 1)
                
                # If not found, use any remaining filtered IP
                if [ -n "$HOME_IP" ]; then
                    HOST_IP="$HOME_IP"
                else
    ...
    if [ -z "$HOST_IP" ]; then
        ...
        echo ""
        echo "⚠️  If this is NOT your WiFi/Ethernet IP, enter the correct one"
        echo "   (Find it with 'ipconfig' on Windows or 'ip addr' on Linux)"
        echo ""
